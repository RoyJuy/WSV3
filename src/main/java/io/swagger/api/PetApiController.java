...
public class PetApiController implements PetApi {
    ...
    @Override
    public ResponseEntity<Void> addPet(@ApiParam(value = "Pet object that needs to be added to the store", required = true) @Valid @RequestParam(value = "addPet") String action1, @RequestParam String userToken, @RequestHeader("Accept") String accept) {
        // do some magic!
        return new ResponseEntity<Void>(HttpStatus.OK);
    }

    @Override
    public ResponseEntity<Void> deletePet(@ApiParam(value = "Pet id to delete", required = true) @RequestParam(value = "deletePet") String action1, @RequestParam int petId, @RequestParam String userToken, @ApiParam(value = "") @RequestHeader(value = "api_key", required = false) String apiKey, @RequestHeader("Accept") String accept) {
        return null;
    }

    @Override
    public ResponseEntity<List<Pet>> findPetsByStatus(@NotNull @ApiParam(value = "Status values that need to be considered for filter", required = true, allowableValues = "available, pending, sold") @RequestParam(value = "getPetItem", required = true) String action1, @RequestParam(required = false) String userToken, @RequestParam int petId, @RequestHeader("Accept") String accept) throws IOException {
        return null;
    }

    @Override
    public ResponseEntity<Void> getPetType(@NotNull @ApiParam(value = "", required = true) @RequestParam(value = "getPetTypeList") String action1) {
        return null;
    }
